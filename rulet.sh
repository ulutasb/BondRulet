...
while IFS='=' read -r key value; do
    case "$key" in
        ...
        BACKUP__BW) BACKUP__BW="$value" ;;
        CREATE_BACKUP_BOND) CREATE_BACKUP_BOND="$value" ;;
       ## BACKUP__GW)BACKUP__GW="$value" ;;
        BACKUP__IP) BACKUP_IP="$value" ;;
       BACKUP__LACP) BACKUP__LACP="$value" ;;
...
# BACKUP Bond'u test etme ve bant genişliği tutmazsa silme fonksiyonu
test_and_cleanup_backup_bond() {
    local bond_name=$1
    local expected_bw=$2
    shift 2
    local nics=("$@")
    ...
    if [[ "$bw" == "$expected_bw" ]]; then
        echo "$bond_name bondu başarıyla oluşturuldu. Bant genişliği doğru: $bw Mb/s"
        return 0
    else
        echo "Bond $bond_name başarısız, beklenen bant genişliği: ${expected_bw} Mb/s, mevcut: $bw Mb/s"
...
for ((i = 0; i < ${#nic_array[@]} - 1; i++)); do
    for ((j = i + 1; j < ${#nic_array[@]}; j++)); do
        ...
        echo "DATA Bond'u deneniyor: ${data_nics[*]}"
        ...
        if test_and_cleanup_bond "$BOND_NAME" "$DATA_BW" "${data_nics[@]}"; then
            ...
            backup_remaining_nics=()
            for nic in "${nic_array[@]}"; do
                if [[  " ${data_nics[@]} " != " ${nic} " ]]; then
                    backup_remaining_nics+=("$nic")
                    echo "nic listesi guncelleniyor"
                fi

            done

            #if [ "$CREATE_BACKUP_BOND" == "evet" ]; then
              for ((i = 0; i < ${#backup_remaining_nics[@]} - 1; i++)); do
                   for ((j = i + 1; j < ${#backup_remaining_nics[@]}; j++)); do
                      data_nics=("${nic_array[i]}" "${nic_array[j]}")
                     echo "BACKUP Bond'u deneniyor: ${data_nics[*]}"

                     create_bond "$BACKUP_BOND_NAME" "$BACKUP_IP" "$BACKUP__GW" "$BACKUP__LACP" "${data_nics[@]}"

                     if test_and_cleanup_backup_bond "$BACKUP_BOND_NAME" "$BACKUP_BW" "${data_nics[@]}"; then
                          echo "Başarılı BACKUP Bond konfigürasyonu bulundu: ${data_nics[*]}"
                          exit 0

                     else
                      echo "DATA BONDU BASARIYLA YAPILANDIRILDI FAKAT BACKUP YAPILANDIRILIRKEN HATA ALINDI!!"
                      exit 1
                     fi
                   done
              done
