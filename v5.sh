...
test_and_cleanup_bond() {
    ...
    local bw=$(ethtool "$bond_name" | grep "Speed" | awk '{print $2}' | sed 's/[a-zA-Z]//g')
    ...
    if [[ "$bw" == "$expected_bw" ]]; then
        ...
        for nic in "${nics[@]}"; do
            # Slave'leri sil
            nmcli con show | grep "$nic" | awk -F: '{print $1}' | xargs -r nmcli con delete
        done
        return 1
    fi
}
...
for ((i = 0; i < ${#nic_array[@]} - 1; i++)); do
    for ((j = i + 1; j < ${#nic_array[@]}; j++)); do
        ...
        if test_and_cleanup_bond "data-bond" "$DATA_BW" "${data_nics[@]}"; then
            ...
            if [ "$CREATE_BACKUP_BOND" == "evet" ]; then
                for ((k = 0; k < ${#remaining_nics[@]} - 1; k++)); do
                    for ((l = k + 1; l < ${#remaining_nics[@]}; l++)); do
                        ...
                        if test_and_cleanup_bond "backup-bond" "$BACKUP_BW" "${backup_nics[@]}"; then
